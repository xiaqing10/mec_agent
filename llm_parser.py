...
| diagnose_device | 执行 | 诊断单台设备（SSH逐步排查，含进程/ROS/图片/传感器等） | ip(必填), project(可选) |
| llm_diagnose | 执行 | LLM深度分析单台设备（代码诊断+LLM根因分析） | ip(必填), project(可选), diag_type(可选) |
| device_info | 执行 | 查询设备具体信息（硬盘占用、内存、CPU、网络、uptime等） | ip(必填), info_type(可选: disk/memory/cpu/network/uptime), project(可选) |
...
6. **上下文指代**：当用户说"详细分析"、"继续"、"统计图片数量"、"roscore怎么样"等，
   如果上下文中提供了最近操作的设备IP或项目名，自动继承到参数中。
   例如：上下文显示最近操作了44.42.122.140，用户说"详细分析" → diagnose_device, ip=44.42.122.140

7. **设备信息查询 (device_info)**：
   - 用户询问某台设备的硬盘/内存/CPU/网络/uptime等具体信息时，用 device_info
   - "硬盘占用率/磁盘空间" → device_info, info_type=disk
   - "内存使用/内存占用" → device_info, info_type=memory
   - "CPU使用率" → device_info, info_type=cpu
   - "网络/带宽" → device_info, info_type=network
   - "运行多久/uptime" → device_info, info_type=uptime
   - 如果上下文中有最近操作的设备IP，自动继承到ip参数
...
严格返回JSON，不要包含其他文字：
{
  ...
  "parameters": {
    "project": "项目名或null",
    "ip": "IP或设备名或null",
    "info_type": "disk|memory|cpu|network|uptime|null",
...
def parse_intent(user_message: str, context_prompt: str = "") -> dict:
    """使用LLM解析用户消息为结构化意图。

    Args:
        user_message: 用户消息
        context_prompt: 上下文提示词（来自会话记忆），拼接到system prompt中
    """
    result = {
        "action": "unknown",
        "parameters": {},
...
def _validate_and_fix(result: dict) -> dict:
    ...
    valid_actions = {"analyze", "diagnose_project", "diagnose_device",
                     ...
                     "llm_analyze", "query_abnormal", "device_info", "unknown"}
